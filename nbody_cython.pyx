...
The nbody_opt version contains all the optimization I made.
runtime: 25.41s 
relative speedup: 4.54x

This is the runtime report for the cython run.
runtime: 3.74s
relative speedup from nbody_opt: 6.79x
...
def advance(float dt, int iterations, dict BODIES):
    '''
        advance the system, one step each iteration, and repeat for all iteration under one loop.
    '''
    cdef list keys = BODIES.keys()
    cdef list pairs = list(itertools.combinations(keys, 2))
    cdef list v1, v2, r
    cdef float x1, x2
    cdef float y1, y2
    cdef float z1, z2
...
def report_energy(dict BODIES, float e=0.0):
    ...
    '''
    cdef list keys = list(BODIES.keys())
    cdef list pairs = list(itertools.combinations(keys, 2))
    cdef list v1, v2, r
    cdef float x1, x2
    cdef float y1, y2
...
def offset_momentum(dict BODIES, list ref, float px=0.0, float py=0.0, float pz=0.0):
    '''
...
def nbody(int loops, str reference, int iterations, dict BODIES):
    '''
        nbody simulation
    ...
    '''
    # Set up global state
    cdef list ref = list(BODIES[reference])
